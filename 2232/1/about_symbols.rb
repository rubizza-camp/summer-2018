...
class AboutSymbols < Neo::Koan
  ...
  def test_method_names_become_symbols
    sym_as_str = Symbol.all_symbols.map(&:to_s)
    assert_equal true, sym_as_str.include?('test_method_names_become_symbols')
  end
  ...
  in_ruby_version('mri') do
    RUBY_CONST = 'What is the sound of one hand clapping?'.freeze
    def test_constants_become_symbols
      all_symbols_as_strings = Symbol.all_symbols.map(&:to_s)

      assert_equal false, all_symbols_as_strings.include?(RUBY_CONST)
    end
  end
